...
class VehicleDetection:
    ...
    def divide_into_grid(self, frame, detected_lights):
        ...
        # Initialize an 8x16 matrix with zeros
        grid = np.zeros((rows, cols), dtype=int)

        # Map detected lights to grid cells
        for (x1, y1, x2, y2) in detected_lights:
            # Calculate the grid cell range affected by the light bounding box
            start_row = max(0, y1 // cell_height)
            end_row = min(rows - 1, y2 // cell_height)
            start_col = max(0, x1 // cell_width)
            end_col = min(cols - 1, x2 // cell_width)

            # Mark the corresponding cells as 1
    ...
    def display(self):
        while self.cap.isOpened() and self.running:
            ...
            # Divide frame into 8x16 grid and get the binary matrix
            grid = self.divide_into_grid(self.frame, detected_lights)
            print("Grid Matrix:\n", grid)
