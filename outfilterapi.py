...
class VehicleDetectionAndGridManipulation:
    ...
    def manipulate_grid(self, grid_coords):
        ...
        for (grid_x, grid_y) in grid_coords:
            merged = False
            for i, (w, x, y, z) in enumerate(merged_boxes):
                # Check for overlap or adjacency
                if not (grid_x > z or grid_x < y or grid_y > x or grid_y < w):
                    # Merge the current box with an existing one
                    merged_boxes[i] = (
                        min(w, grid_y - 1),
                        max(x, grid_y + 1),
                        min(y, grid_x - 1),
            ...
            if not merged:
                ...
                merged_boxes.append((w, x, y, z))

        # Ensure only two boxes are sent to the server
        if len(merged_boxes) > 2:
            merged_boxes = merged_boxes[:2]
    ...
    def display(self):
        while self.cap.isOpened() and self.running:
            ...
            with self.lock:
                ...
                # Print both filtered and unfiltered data
                print("Unfiltered Data:", vehicle_grid_coords)
                print("Filtered Data (Manipulated):", self.manipulated_data)
...
video_path = "/home/coralfallacy/Desktop/circuitbreaker/dashcam.mp4"
